...
void Riad_FL(RGZ* h)
{
	std::regex regex_integer(("^[А-ЯA-Z][А-Яа-яa-zіA-Z -_\.,]{2,16}$"));//проверка на великий символ
	std::regex regex_integer2(("^[А-Яа-яA-za-z1-9і][А-Яа-яA-za-z0-9]{1,10}([.,]?){0,1}([-_\ ]?){0,1}[А-Яа-яA-za-z]{1,10}$"));//перевірка на правильний ввод
...
void SR::checking_the_constructor()
{
	...
	std::cout << "елемент класу: " << konst.Get_teme() << std::endl;
	SR konst2;//для конструктора
	std::cout << "елемент класу: " << konst2.Get_teme() << std::endl;
	SR konst3(konst2);//для конструктора копіювання
	std::cout << "елемент класу : " << konst3.Get_teme() << std::endl;
}
...
{
	std::regex regex_integer("^[А-Яа-яA-za-z0-9]{1,10}([.,]?){0,1}[-_\ ][А-Яа-яA-za-z]{1,10}$");
