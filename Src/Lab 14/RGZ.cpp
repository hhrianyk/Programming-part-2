...
template<typename T> void RGZ::Sort(int N, vector<T>& A, set<T>& B, list<T>& C, map<int, T>& D, vector<RGZ*>& E)
{
	if (N == 1)if (A.empty())
	{
		cout << "!!!!Пустий масив!!!" << endl;
		...
		sort(A.begin(), A.end(), Comparison<T>);
	}
	else if (N == 2)if (B.empty())
	{
		cout << "!!!!Пустий масив!!!" << endl;
		...
		//sort(B.begin(), B.end(), Comparison<T>);
		cout << "Ваш set вже відсортований" << endl;
	}
	else if (N == 3)if (C.empty())
	{
		cout << "!!!!Пустий масив!!!" << endl;
	...
	else if (N == 4)if (D.empty())
	{
		...
		cout << "Ваш map вже відсортований?" << endl;
	...
	else if (N == 5)if (E.empty())
	{
		...
		sort(E.begin(), E.end(), Comparison<RGZ*>);
	}


}

template<typename T> void RGZ::insert_vector(int N, vector<T>& A, vector<RGZ*>& E)
{
	int t=0;
	if (N != 1 && N != 5)
	{
		cout << "Лише об’єднання двох STL контейнерів типу vector" << endl;
			return;
	}
	else if (N == 1)
	{
		T add;
		vector<T> B;
		cout << "Створіть другий векторо з 2 елементів"<<endl;
		for (int i = 0; i < 2; i++)
		{
			el(add);
			B.push_back(add);
		...
		while (t != -1)
		{
			...
			else cout << "\nПомилкка спробуй ще раз" << endl;
			system("pause&cls");
		}
		cout << "успішно зроблено";
	}
	else if (N == 5)
	{
		T add;
		vector<RGZ*> B;
		cout << "Створіть другий векторо з 2 елементів";
		for (int i = 0; i < 2; i++)
		{
			el(add);
			B.push_back(new T(add));
		}
		while (t!=-1)
		{
			cout << "Вкажіть позицію :";
		...
		if (t > 0 && t <= E.size())
		{
			E.insert(E.begin() +t, B.begin(), B.end());
			t = -1;
		}
		else cout << "Помилкка спробуй ще раз" << endl;
		system("pause&cls");
		}
	}
}
 ...
 int RGZ::vstup_info(number nov, int N, vector<RGZ>& A, set<RGZ>& B, list<RGZ>& C, map<int, RGZ>& D, vector<RGZ*>& E)
{
	...
	while (a)
	{
		...
		case 7:h.Poshuk(N, A, B, C, D, E);
			break;
		case 8: h.Sort(N, A, B, C, D, E);
			break;
		case 9:h.insert_vector(N, A, E);
			break;
		case 11: system("cls");
			system("cls");
			nov = 3;
 ...
 void RGZ::additional_tasks()
 {
	 int x, y = 2;
	 RGZ add;
	 string key,predmet;
	 map<string, set<RGZ>> m1, m2, m3;
	 set<RGZ> a1, a2, a3;
	 cout << "Скільки груп хочете додати у перший мар :";
	 cin >> x;
	 for (int i = 0; i < x; i++)
	 {
		 cout << "Введіть ключ(група) :";
		 cin >> key;
		 for (int j = 0; j < y; j++)
		 {	 
			 cout << "Введіть назву предмету:";
			 cin >> predmet;
			 add.Set_predmet(predmet);
		 a1.insert(add);
		 }

		 m1.insert(pair<string, set<RGZ>>(key, a1));
	 }
	 cout << endl << endl;
	 cout << "Скільки елементів хочете додати у другий мар :";
	 cin >> x;
	 ...
	 for (int i = 0; i < x; i++)
	 {
		 ...
		 m2.insert(pair<string, set<RGZ>>(key, a2));
	 }
	 cout << endl<<endl;
	 m3 = m1;
	 for_each(m2.begin(), m2.end(), [&](const pair<string, set<RGZ>>& pair) {
	 ...
	 for (map<string, set<RGZ>>* m : std::array<map<string, set<RGZ>>*, 3>{&m1, & m2, & m3}) {
		 cout << endl<<endl;
		 for (pair<string, set<RGZ>> k : *m) {
			 cout << k.first << ":";
			 for (RGZ s : k.second)
				 cout << s.Get_predmet() << ",";
			 cout << ";"<<endl;
